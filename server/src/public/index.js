...
if (typeof window.ethereum !== 'undefined') {
  ...
  async function getInsuranceContractBalance() {
    const contractBalance = await insuranceContract.methods.getInsruanceBalance().call({
  ...
  async function purchaseInsurance() {
    const address = await getAccount();
    const insuranceData = await insuranceContract.methods.insurancePayment().send({
      from: address,
  ...
  async function expireInsurance() {
    const address = await getAccount();
    const withdrawInsurance = insuranceContract.methods.withdrawForclient(1000000).send({
      from: address,
    });
